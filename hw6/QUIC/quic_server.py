from QUIC.common.packet import Packet, Frame
from QUIC.common.helper import Buffer
import socket
...
class QUICServer:
    ...
    def accept(self):
        ...
        logging.info(f"accepted {self.client_addr} connection")
    ...
    def meter_task(self):
        while self.is_running:
            logging.info(f"{self.byte_counter * 8}bps")
            self.byte_counter = 0
            time.sleep(1)
        print('meter out')
    ...
    def close(self):
        ...
        self.send_manager.join()
        self.receive_manager.join()
